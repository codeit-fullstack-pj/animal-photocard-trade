...
├─ routes/            # URL → 컨트롤러 연결 (auth.routes.js, health.routes.js, image.routes.js, card.routes.js)
├─ controllers/       # 요청 본문 검사(superstruct), 응답 형태 결정 (auth.controller.js, image.controller.js, card.controller.js)
├─ services/          # 비즈니스 로직, Supabase·DB 조합 (auth.service.js, image.service.js, card.service.js)
├─ repositories/      # Prisma 쿼리 (user.repository.js, image.repository.js, card.repository.js)
├─ middlewares/       # error-handler, csrf, require-auth (로그인 여부 확인, req.user 채움)
└─ lib/               # prisma, supabase 클라이언트, ApiError, score-config(관상 지수 축), score-model(ML 추론), card-flavor(태그 자동 생성)
```
...
- `test/cards.http`: 카드 생성 (인증·CSRF 필요, 201·400·401·403·404·409)
