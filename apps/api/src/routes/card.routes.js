...
cardRouter.get("/", requireAuth, cardController.list);
// POST /api/v1/cards — 업로드된 이미지(imageId)로 포토카드 생성
cardRouter.post("/", requireAuth, requireCsrfToken, cardController.create);
// GET /api/v1/cards/remaining-count — 오늘 남은 카드 생성 가능 횟수
cardRouter.get("/remaining-count", requireAuth, cardController.remainingCount);
